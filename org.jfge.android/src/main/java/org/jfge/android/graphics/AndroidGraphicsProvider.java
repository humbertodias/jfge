...
@Singleton
public class AndroidGraphicsProvider extends SurfaceView
    implements GraphicsProvider, SurfaceHolder.Callback {
   ...
   */
  @Inject
  public AndroidGraphicsProvider(
      Logger logger,
      Activity activity,
  ...
  public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
    ...
    if (height > width) {
      float scaleFactor =
          Math.min(((float) width / dbImage.getWidth()), ((float) height) / dbImage.getHeight());
      float translateFactor = (height - scaleFactor * dbImage.getHeight()) / 2;
      matrix.postScale(scaleFactor, scaleFactor);
      matrix.postTranslate(0, translateFactor);
    } else {
      matrix.postScale(
          ((float) width) / dbImage.getWidth(), ((float) height) / dbImage.getHeight());
    }
