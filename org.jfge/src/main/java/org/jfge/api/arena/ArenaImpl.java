...
public final class ArenaImpl implements Arena {
  ...
  private void updateFighterDirection() {
    ...
    if (Math.abs(leftFighter.getX() - rightFighter.getX())
        < (leftFighter.getImage().getWidth() + rightFighter.getImage().getWidth()) / 2) {
  ...
  public void render(Graphics graphics) {
    ...
    if (leftFighter != null) {
      this.arenaRenderer.drawLifebar(
          graphics,
          leftFighter.getName(),
          leftFighter.getHealth(),
          leftFighter.getPortrait(),
          this.leftFighterWins,
          ArenaRenderer.ALIGN_LEFT);
    }

    if (rightFighter != null) {
      this.arenaRenderer.drawLifebar(
          graphics,
          rightFighter.getName(),
          rightFighter.getHealth(),
          rightFighter.getPortrait().flip(),
          this.rightFighterWins,
          ArenaRenderer.ALIGN_RIGHT);
    }
  ...
  public Fighter getWinner() {
    ...
    if (this.rightFighter.hasFinalStateReached()) return this.leftFighter;

    if (this.leftFighter.hasFinalStateReached()) return this.rightFighter;
  ...
  public boolean hasWinner() {
    if (this.leftFighter == null) return false;

    if (this.rightFighter == null) return false;

    if (this.rightFighter.hasFinalStateReached()) return true;

    if (this.leftFighter.hasFinalStateReached()) return true;
  ...
  public void setFigherLeftPos(int x, int y) {
    if (this.leftFighter == null) return;
  ...
  public void setFighterRightPos(int x, int y) {
    if (this.rightFighter == null) return;
